...
namespace IM.Service.MarketData.Services.Calculations;
...
public class CoefficientService
{
    ...
    public async Task SetCollectionAsync<T>(string data) where T : class, IDataIdentity
    {
        if (!RabbitHelper.TrySerialize(data, out T[]? identities))
            throw new SerializationException(typeof(T).Name);

        var coefficients = identities switch
        {
        ...
        var (error, _) = await coefficientRepo
            .CreateUpdateAsync(coefficients, new DataQuarterComparer<Coefficient>(), nameof(SetCollectionAsync));
