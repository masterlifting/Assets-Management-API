...
using IM.Service.Data.Models.Api.Amqp;
...
{
    private readonly Repository<Company> repository;
    ...
    public async Task<ResponseModel<CompanyGetDto>> GetAsync(string companyId)
    {
        var company = await repository.FindAsync(companyId.Trim().ToUpperInvariant())
                      ?? await repository.FindAsync(x => x.Name.Contains(companyId)); //TODO: TO DELETE
    ...
    public async Task<ResponseModel<PaginatedModel<CompanyGetDto>>> GetAsync(HttpPagination pagination)
    {
        ...
        if (companies.Any())
        {
            ...
            foreach (var company in companies)
                company.Sources = sourceDictionary.ContainsKey(company.Id)
                    ? sourceDictionary[company.Id].Select(x => new EntityTypeGetDto(x.Name, x.Value)).ToArray()
    ...
    public async Task<ResponseModel<string>> CreateAsync(IEnumerable<CompanyPostDto> models)
    {
        ...
        var (error, createdEntities) = await repository.CreateAsync(entities, new CompanyComparer(), $"Source count: {entities.Length}");
