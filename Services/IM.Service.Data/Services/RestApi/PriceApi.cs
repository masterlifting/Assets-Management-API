...
public class PriceApi : IRestService<Price, PriceGetDto>
{
    private readonly IRestQueryService<Price> queryService;
    ...
    public PriceApi(
        IRestQueryService<Price> queryService,
    ...
    public async Task<ResponseModel<PriceGetDto>> GetAsync(string companyId, DateOnly date)
    {
        ...
        var Splits = await stockSplitRepository.GetSampleAsync(x => x.CompanyId == company.Id && x.Date <= date);
        ...
        return new()
        {
            Data = new()
            {
                ...
                Value = price.Value,
                ValueTrue = !Splits.Any() ? price.Value : price.Value * Splits.Aggregate(1, (current, item) => current * item.Value),
                StockVolume = !stockVolumes.Any() ? default : stockVolumes[^1].Value
            }
        };
    }
    public Task<ResponseModel<PaginatedModel<PriceGetDto>>> GetAsync<T>(T filter, HttpPagination pagination) where T : class, IFilter<Price>
    {
        throw new NotImplementedException();
    }
    public async Task<ResponseModel<PaginatedModel<PriceGetDto>>> GetAsync(DateFilter<Price> filter, HttpPagination pagination)
    {
        ...
        if (queryResult.Any())
        {
            ...
            var Splits = await stockSplitRepository.GetSampleAsync(x => companyNamesDictionary.Keys.Contains(x.CompanyId) && x.Date <= date);
            ...
            foreach (var groupSplits in Splits.GroupBy(x => x.CompanyId))
            {
    ...
    public async Task<ResponseModel<PaginatedModel<PriceGetDto>>> GetLastAsync(CompanySourceDateFilter<Price> filter, HttpPagination pagination)
    {
    ...
    public async Task<ResponseModel<string>> CreateAsync(IEnumerable<PricePostDto> models)
    {
        ...
        var entities = dtos.Select(x => new Price
        {
        ...
        var (error, result) = await priceRepository.CreateAsync(entities, new CompanyDateComparer<Price>(), $"Source count: {entities.Length}");
