...
namespace IM.Service.Data.Services.DataFounders.Reports.Implementations;
...
public class InvestingGrabber : IDataGrabber
{
    ...
    public async Task GrabCurrentDataAsync(IEnumerable<DataConfigModel> configs) =>
        await GrabHistoryDataAsync(configs);
    ...
    public async Task GrabHistoryDataAsync(DataConfigModel config)
    {
        try
        {
            ...
            var result = InvestingParserHandler.Parse(data, config.CompanyId);
