...
public class DateControllerBaseLevel1<TEntity, TGet> : ControllerBase where TGet : class where TEntity : class, IDataIdentity, IDateIdentity
{
    ...
    [HttpGet("{companyId:string}/{sourceId:int}/{year:int}/{month:int}/{day:int}")]
    ...
    #region Get More
    public async Task<ResponseModel<PaginatedModel<TGet>>> Get(int year = 0, int quarter = 0, int day = 0, int page = 0, int limit = 0) =>
        await api.GetAsync(new DateFilter<TEntity>(HttpRequestFilterType.More, year, quarter, day), new(page, limit));
    [HttpGet("last/")]
    public async Task<ResponseModel<PaginatedModel<TGet>>> GetLast(int year = 0, int quarter = 0, int day = 0, int page = 0, int limit = 0) =>
        await api.GetLastAsync(new DateFilter<TEntity>(HttpRequestFilterType.More, year, quarter, day), new(page, limit));
    #endregion
