...
namespace IM.Service.Company.Reports.Services.ReportServices.Implementations
{
    public class InvestingParser : IReportParser
    {
        ...
        public async Task<Report[]> GetReportsAsync(string source, ReportLoaderData data)
        {
            ...
            try
            {
                var site = await handler.LoadSiteAsync(data.SourceValue);
                result = InvestingParserHandler.ParseReports(site, data.TickerName, source);
            }
            catch (Exception ex)
            {
        ...
        public async Task<Report[]> GetReportsAsync(string source, IEnumerable<ReportLoaderData> data)
        {
            ...
            var result = new List<Report>(_data.Length * 5);
    ...
    internal class InvestingParserHandler
    {
        ...
        internal static Report[] ParseReports(IReadOnlyList<HtmlDocument> site, string ticker, string source)
        {
