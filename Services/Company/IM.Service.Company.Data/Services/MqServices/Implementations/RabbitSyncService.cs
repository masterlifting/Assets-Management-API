...
public class RabbitSyncService : IRabbitActionService
{
    ...
    private async Task<bool> GetCompanyResultAsync(QueueActions action, string data)
    {
        ...
        if (action == QueueActions.Delete)
            return (await companyRepository.DeleteAsync(new object[] { data }, $"{nameof(GetCompanyResultAsync)}.{data}")).error is not null;
        ...
        if (!await GetActionAsync(companyRepository, action, new[] { company.Id }, company, $"{nameof(GetCompanyResultAsync)}.{company.Name}"))
            return false;
        ...
        var cstRepository = scopeFactory.CreateScope().ServiceProvider.GetRequiredService<Repository<CompanySourceType>>();
        return await GetActionsAsync(cstRepository, action, companySourceTypes, new CompanySourceTypeComparer(), $"{nameof(GetCompanyResultAsync)}.Sources for '{company.Name}'");
    }
    ...
    {
        QueueActions.Create => (await repository.CreateAsync(data, comparer, value)).error is null,
