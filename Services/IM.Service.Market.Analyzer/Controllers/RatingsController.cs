...
namespace IM.Service.Market.Analyzer.Controllers;
...
public class RatingsController : ControllerBase
{
    ...
    public async Task<string> Recalculate(string companyId, int year, int month, int day)
    {
        var companyIds = companyId.Split(',');
        return companyIds.Length > 1
            ? await manager.RecalculateAsync(new CompanyDataFilterByDate<Rating>(Common.Net.Enums.HttpRequestFilterType.Equal, companyIds, year, month, day))
            : await manager.RecalculateAsync(new CompanyDataFilterByDate<Rating>(Common.Net.Enums.HttpRequestFilterType.Equal, companyId, year, month, day));
    }
    ...
    public async Task<string> Recalculate(int year, int month, int day) =>
        await manager.RecalculateAsync(new CompanyDataFilterByDate<Rating>(Common.Net.Enums.HttpRequestFilterType.Equal, year, month, day));
}