...
public class StockSplitsDtoManager
{
    ...
    public async Task<ResponseModel<string>> CreateAsync(IEnumerable<StockSplitPostDto> models)
    {
        ...
        var (error, result) = await stockSplitRepository.CreateAsync(entities, new CompanyDateComparer<StockSplit>(), $"Source count: {entities.Length}");
    ...
    public string Load()
    {
        ...
        publisher.PublishTask(QueueNames.MarketData, QueueEntities.StockSplits, QueueActions.Call, DateTime.UtcNow.ToShortDateString());
    ...
    public string Load(string companyId)
    {
        companyId = companyId.Trim().ToUpperInvariant();
        var publisher = new RabbitPublisher(rabbitConnectionString, QueueExchanges.Function);
        publisher.PublishTask(QueueNames.MarketData, QueueEntities.StockSplit, QueueActions.Call, companyId);
