...
public class RestApiWrite<TEntity, TPost> where TPost : class where TEntity : class, IDataIdentity, IPeriod
{
    ...
    public async Task<string> LoadAsync()
    {
        ...
        publisher.PublishTask(QueueNames.Market, sources[entityName].multiply, QueueActions.Get, companySources);
    ...
    public async Task<string> LoadAsync(string companyId, byte sourceId)
    {
        ...
        var publisher = new RabbitPublisher(rabbitConnectionString, QueueExchanges.Function);
        publisher.PublishTask(QueueNames.Market, sources[entityName].single, QueueActions.Get, companySource);
