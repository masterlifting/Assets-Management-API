...
namespace IM.Services.Companies.Reports.Api.Services.Background.Implementations
{
    public class InvestingUpdater : IClientReportUpdater
    {
        private readonly InvestingClient client;

        public InvestingUpdater(InvestingClient client)
        {
            this.client = client;
        }
        ...
        private static Report[] GetReports(HtmlDocument[] site, int reportSourceId)
        {
            ...
            for (int i = 0; i < 4; i++)
                result[i] = new()
                {
                    ...
                    Quarter = ReportUpdater.GetQuarter(financialPage.Dates[i].Month),
                    ...
                    if (dividendDates.Any())
                    {
                        ...
                        if (dividendValueData is not null)
                        {
                            ...
                            for (int i = 0; i < dates.Length; i++)
                            {
                                ...
                                var reportQuarter = ReportUpdater.GetQuarter(dates[i].Month);

                                for (int j = 0; j < dividendDates.Count; j++)
                                {
                                    var dividendYear = dividendDates[j].Year;
                                    var dividendQuarter = ReportUpdater.GetQuarter(dividendDates[j].Month);
                                    ...
                                    if (reportYear == dividendYear && reportQuarter == dividendQuarter)
                                    {
                                        ...
                                        if (decimal.TryParse(dividendValue, out decimal data))
                                        {
                                            if (j > 0)
                                            {
                                                ...
                                                var previousDividendQuarter = ReportUpdater.GetQuarter(dividendDates[j - 1].Month);
